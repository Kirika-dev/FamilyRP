				...
				try {
					if (player.handle !== 0 && player !== global.localplayer) {
						...
						if(global.adminLVL == 9 || global.adminLVL >= Number (playerALVL)) {
							...
							console.log(`fractionId: ${fractionId}`); // Добавлено для проверки значения fractionId
		...
		if(global.esptoggle == 2 || global.esptoggle == 3) {
			mp.vehicles.forEachInStreamRange(vehicle => {
				if (vehicle.handle !== 0 && vehicle !== global.localplayer.vehicle) {
					...
					try {
						let vehicleName = mp.game.vehicle.getDisplayNameFromVehicleModel(vehicle.model);
						let vehicleNumberPlate = vehicle.getNumberPlateText();
						let distance = Math.round(mp.game.system.vdist(position.x, position.y, position.z, localPosition.x, localPosition.y, localPosition.z));
						let vehicleHealth = parseInt(vehicle.getEngineHealth());
						let vehicleSpeed = parseInt(vehicle.getSpeed() * 3.6); // Конвертируем скорость в км/ч

						let nameTag = `${vehicleName} ~r~(${vehicle.remoteId}) ~w~ | ${vehicleNumberPlate}\n${distance} M | ${vehicleHealth} HP | ${vehicleSpeed} KM3H`;

						mp.game.graphics.drawText(nameTag, [position.x, position.y, position.z - 0.5], {
							scale: [0.3, 0.3],
							outline: true,
							color: [255, 255, 255, 150],
							font: 0,
							shadow: true, // Добавляем тень
							border: true, // Добавляем рамку
							borderColor: [0, 0, 0, 255] // Цвет рамки (черный)
						});
					} catch(e) {
						mp.game.graphics.drawText(`[ESP-ERROR] Cant render vehicle`, [0.20, 0.75], {
							font: 0,
							color: [255, 255, 255, 185],
							scale: [0.35, 0.35],
							outline: true
