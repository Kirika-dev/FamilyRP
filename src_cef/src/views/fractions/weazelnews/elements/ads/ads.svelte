...
{#if advert}
    <button class="weazelnews__back" on:click={onBackAdvert} on:keydown={onBackAdvert}>
    ...
    <div class="box-between mt-20">
        ...
        <button class="weazelnews__button" on:click={onDeleteAdverts} on:keydown={onDeleteAdverts}>
    ...
    <div class="weazelnews__button mt-12" on:click={onSendAnswer} on:keydown={onSendAnswer}>{translateText('fractions', 'Опубликовать')}</div>
{/if}