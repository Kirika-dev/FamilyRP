...
{:else}
    <div in:fade class="box-100">
        <button class="newphone__project_button" on:click={() => onSelectedView("Add")} on:keydown={(e) => { if (e.key === 'Enter') onSelectedView("Add") }}>
            {translateText('player2', 'Подать объявление')}
        </button>
        {#if newsList && typeof newsList === "object" && newsList.length > 0}
            <div class="box-center box-between newphone__project_padding20">
                ...
                {#if !isTypeFilter.length}
                    <button class="newphone__button_filter box-center" on:click={openFilter}>
                        <span class="phoneicons-filter"></span>
                        {translateText('player2', 'Фильтры')}
                    </button>
                {:else}
                    <button class="newphone__button_filter box-center red__background" on:click={openFilter}>
                        {isTypeFilter.length} {getFilterWord(isTypeFilter.length)}
                        <button class="phoneicons-close" on:click={clearFilter} on:keydown={(e) => { if (e.key === 'Enter') clearFilter(e) }} tabindex="0"></button>
                    </button>
                {/if}
            </div>
                            ...
                            <div class="box-between">
                                ...
                                <div class="box-flex">
                                    <div class="newphone__ads_circle">
                                        <button class="phoneicons-call" on:click={() => onCall (item.number)}></button>
                                    </div>
                                    <div class="newphone__ads_circle">
                                        <button class="phoneicons-chat" on:click={() => onMessage (item.number)}></button>
                                    </div>
                                </div>
                            </div>
                        </div>
