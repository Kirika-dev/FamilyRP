    ...
    class Manager : Script
    {
        ...
        public static void organizationChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                string orgColor = organizationData.Color.Alpha != 0 ? $"!{{#{organizationData.Color.Red:X2}{organizationData.Color.Green:X2}{organizationData.Color.Blue:X2}}}" : "!{#50C878}";
                string msgSender = orgColor + $"[FC]{GetDepartmentTag(organizationData.Id, memberOrganizationData.DepartmentId)} " + status + player.Name.Replace('_', ' ') + " (" + player.Value + "): " + Commands.RainbowExploit(message);
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
