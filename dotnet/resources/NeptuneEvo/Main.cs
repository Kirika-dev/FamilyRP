...
namespace NeptuneEvo
{
    public class Main : Script
    {
        ...
        public static Dictionary<Player, CharacterData> Player = new Dictionary<Player, CharacterData>(); // character in
        ...
        public static void SetBlipTransparency(Vector3 position, int transparency)
        {
            var blip = NAPI.Pools.GetAllBlips().FirstOrDefault(b => b.Position == position);
            if (blip != null)
            {
                blip.Transparency = transparency;
            }
        }

        public static void HideBusinessBlip(Vector3 position)
        {
            var blip = NAPI.Pools.GetAllBlips().FirstOrDefault(b => b.Position == position);
            if (blip != null)
            {
                ...
                if (resultOthervehicles != null)
                {
                    ...
                    foreach (DataRow Row in resultOthervehicles.Rows)
                    {
                        try
                        {
                            ...
                            model = NAPI.Util.VehicleInfoToModel(name);
        ...
        public static (byte, float) GetPlayerJobLevelBonus(sbyte jobId, int points)
        {
            try
            {
                switch (jobId)
                {
                    ...
                    case 4://bus
                        if (points >= 3 && points <= 9) return (1, 1.05f);
                        else if (points >= 10 && points <= 19) return (2, 1.1f);
                        else if (points >= 20 && points <= 34) return (3, 1.12f);
                        else if (points >= 35 && points <= 69) return (4, 1.15f);
                        else if (points >= 70) return (5, 1.2f);
                        else return (0, 1);
                    case 5://mech
        ...
        public static int[] GetPlayerJobsNextLevel(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData == null) return new int[] { 700, 2000, 200, 25, 3, 10, 30, 150 };

                int[] nextLevelInfo = { 700, 2000, 200, 25, 3, 10, 30, 150 };
                    ...
                    if (characterData.JobSkills.ContainsKey(i))
                    {
                        ...
                        switch (i)
                        {
                            ...
                            case 3:
                                ...
                                break;
                            case 4:
                                if (points >= 3 && points <= 9) correctValue = 10;
                                else if (points >= 10 && points <= 19) correctValue = 20;
                                else if (points >= 20 && points <= 34) correctValue = 35;
                                else if (points >= 35) correctValue = 70;
                                else correctValue = 3;
                                break;
                            case 5:
            ...
            catch (Exception e)
            {
                ...
                return new int[] { 700, 2000, 200, 25, 3, 10, 30, 150 };
            }
        }
        ...
        [RemoteEvent("keyPressE")]
        public void OnKeyPressE(ExtPlayer player)
        {
            if (player.HasData("nearDoors"))
            {
                List<string> doorIds = player.GetData<List<string>>("nearDoors");
                bool newState = !DoorsControl[doorIds[0]]; // Предполагаем, что все двери в группе имеют одинаковое состояние
                foreach (var doorId in doorIds)
                {
                    if (DoorsControl.ContainsKey(doorId))
                    {
                        DoorsControl[doorId] = newState;
                        Trigger.ClientEventForAll("doorControl", doorId, newState);
                    }
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, newState ? "Двери открыты" : "Двери закрыты", 3000);
            }
        }
        ...
        public Main()
        {
            ...
            try
            {
                ...
                */
                ...
                {
                    NAPI.Util.ConsoleOutput("🚀 Сервер запущен! Команда /getdlcvehicles доступна.");
                }
            }
            catch (Exception e)
            {
    ...
    public class Trigger : Script
    {
        ...
        internal static void ClientEvent(Player player)
        {
