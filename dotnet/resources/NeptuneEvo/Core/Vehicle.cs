...
namespace NeptuneEvo.Core
{
    class VehicleManager : Script
    {
        ...
        public static int GetWeightVeh(string name)
        {
            return WeightCapacity[name];
        }
            ...
            try
            {
                ...
                if (player.VehicleSeat == (int)VehicleSeat.Driver)
                {
                    ...
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        if (vehicleLocalData.Fraction == (int)Fractions.Models.Fractions.ARMY && (vehicle.Model == (uint)VehicleHash.Barracks || vehicle.Model == (uint)VehicleHash.Brickade || vehicle.Model == (uint)VehicleHash.Cargobob || vehicle.Model == (uint)VehicleHash1.mi8cargo))
                        {
            ...
            try
            {
                if (veh != null)
                {
                    ...
                    if (!VehicleModel.AirAutoRoom.isAirCar(veh.Model))
                    {
                        ...
                        veh.TrimColor = data.TrimColor.ToInt32();
        ...
        private static List<string> whitelistNumber = new List<string>
        {
          "77","99","97","50","90","78","98"
        };
        ...
        public static string GenerateNumber()
        {
            ...
            do
            {
                ...
                for (int i = 0; i < 2; i++) number += whitelist[Rnd.Next(0, whitelist.Count)];
                for (int i = 0; i < 2; i++) number += (char)Rnd.Next(48, 57);
        ...
        public static string GenerateNumber(VehicleAccess vehicleAccess, string prifix)
        {
            ...
            do
            {
                ...
                for (int i = 0; i < 2; i++) number += whitelist[Rnd.Next(0, whitelist.Count)];
                for (int i = 0; i < 3; i++) number += (char)Rnd.Next(48, 57);
                number += whitelistNumber[Rnd.Next(0, whitelistNumber.Count)];

            } while (NeptuneEvo.VehicleData.LocalData.Repository.VehicleNumberToHandle[vehicleAccess].ContainsKey(number));
            return number;
        }
        public static string GenerateNumber1(VehicleAccess vehicleAccess, string postfix)
        {
            string number1;
            do
            {
                number1 = postfix;
                number1 += whitelist[Rnd.Next(0, whitelist.Count)];
                for (int i = 0; i < 3; i++) number1 += (char)Rnd.Next(48, 57);
                for (int i = 0; i < 2; i++) number1 += whitelist[Rnd.Next(0, whitelist.Count)];
                number1 += whitelistNumber[Rnd.Next(0, whitelistNumber.Count)];

            } while (NeptuneEvo.VehicleData.LocalData.Repository.VehicleNumberToHandle[vehicleAccess].ContainsKey(number1));
            return number1;
        }
        ...
        internal static string Create(string name, string vName, Color color1, Color color2, Color color3)
        {
