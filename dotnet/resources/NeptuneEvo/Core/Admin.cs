    ...
    class Admin : Script
    {
        ...
        public static void checkMoney(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) проверил денежные средства {target.Name} ({target.Value})");
        ...
        public static void giveTargetClothes(ExtPlayer player, ExtPlayer target, int clothingId, int drawable, int texture)
        {
            try
            {
                ...
                ItemId itemType = (ItemId)clothingId;
                if (Chars.Repository.ItemsInfo[itemType].functionType != newItemType.Clothes)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Этой командой можно выдавать только предметы одежды", 3000);
                    return;
                }

                if (Chars.Repository.AddNewItem(target, $"char_{targetCharacterData.UUID}", "inventory", itemType, 1, $"{drawable}_{texture}") == -1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У игрока недостаточно места в инвентаре", 3000);
                    return;
                }

                GameLog.Admin($"{player.Name}", $"giveClothes({clothingId},{drawable},{texture})", $"{target.Name}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы выдали игроку {target.Name} одежду (ID: {clothingId}, Drawable: {drawable}, Texture: {texture})", 3000);
            }
            catch (Exception e)
            {
