...
namespace NeptuneEvo.Core
{
    public class VehicleStreaming : Script
    {
        ...
        public GTANetworkAPI.Vehicle CreateVehicle(VehicleHash model, Vector3 pos, Vector3 rot, int color1, int color2, string numberPlate = "", byte alpha = byte.MaxValue, bool locked = false, bool engine = true, uint dimension = 0u)
        {
            return CreateVehicle((uint)model, pos, rot.Z, color1, color2, numberPlate, alpha, locked, engine, dimension);
        }
        public static ExtVehicle CreateVehicle1(VehicleHash model, Vector3 pos, Vector3 rot, int color1, int color2, string numberPlate = "", byte alpha = 255, bool locked = false, bool engine = false, uint dimension = 0, int petrol = 0, int workdriv = -1, bool cm = false, bool cd = false, bool cmk = false, string by = "NONE", VehicleAccess acc = VehicleAccess.None, int fr = -1, int numb = -1, JobsId work = JobsId.None, float dirt = 0.0f, int minrank = 100)
        {
            try
            {
                var veh = (ExtVehicle)NAPI.Vehicle.CreateVehicle((uint)model, pos, rot.Z, color1, color2, numberPlate, alpha, locked, engine, dimension);
