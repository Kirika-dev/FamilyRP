...
namespace NeptuneEvo.Core
{
	class BusinessManager : Script
	{
			...
			try
			{
				...
				foreach (Product prod in data.Products)
				{
					...
					else
					{
						...
						else if (data.Type == 9) minPrice = Main.PricesSettings.ClothesMinPrice;
						else if (data.Type == 11 || data.Type == 12 || data.Type == 13 || data.Type == 14) minPrice = Main.PricesSettings.TattooBarberMasksLscMinPrice;
						...
						else if (data.Type == 9) maxPrice = Main.PricesSettings.ClothesMaxPrice;
						else if (data.Type == 11 || data.Type == 12 || data.Type == 13 || data.Type == 14) maxPrice = Main.PricesSettings.TattooBarberMasksLscMaxPrice;
				...
				switch (data.Type)
				{
					...
					case 5:
						...
						break;
					case 6:
						foreach (string name in CarsNames[4])
						{
							if (!BusProductsData.ContainsKey(name)) continue;
							else if (BusProductsData[name].Price == 0) continue;
							if (data.Products.FirstOrDefault(x => x.Name == name) == null)
		...
		public static int[] BlipByType = new int[17]
		{
			...
			662, // Japan Autoroom
			663, // China Autoroom
			664, // Germany Autoroom
			522, // moto
			665, // Russian Autoroom
			666, // Central Asia Autoroom
			567, // gun shop
			366, // clothes shop
			628, // burger-shot
			75, // tattoo-salon
		...
		{
			new List<string>() // Japan Autoroom
			{
		 "brz13",
		 "lc100",
		 "lc300gr",
		 ...
		 "mlevox"
			...
			new List<string>() // China Autoroom
			{
			"ehs9",
			"wey500",
			"2024lixiangl9",
			...
			new List<string>() // Germany Autoroom
			{
                "macan",
             "bmwe39",
		"bmwm5e392002wb",
        ...
        "Dubsta3",
			...
			new List<string>() // Central Asia Autoroom
			{
		"bug300ss",
        "btourbillon24",
        "c8",
        ...
        public static void fillCar(ExtPlayer player, int lvl)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    // Проверка на принадлежность к армии
                    if (fractionData != null && fractionData.Id == (int)Fractions.Models.Fractions.ARMY && (sessionData.BizID == 108 || sessionData.BizID == 112))
                    {
                        price = 10; // Бесплатная заправка для армии
                        if (vehicleLocalData.Access == VehicleAccess.Fraction && vehicleLocalData.Fraction == (int)Fractions.Models.Fractions.ARMY)
                        {
                            price = 0; // Бесплатная заправка для армии
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Только машины, принадлежащие Армии, могут заправляться бесплатно.", 3000);
                            return;
                        }
                    }
                    else if (isGov)
		...
		public static void acceptBuyBusiness(ExtPlayer player)
		{
			try
			{
				...
				Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы купили у {target.Name.Replace('_', ' ')} {BusinessTypeNames[biz.Type]} за {Wallet.Format(price)}₽", 3000);
				Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{player.Name.Replace('_', ' ')} купил у Вас {BusinessTypeNames[biz.Type]} за {Wallet.Format(price)}₽", 3000);
        ...
        public static void HideBusinessBlip(int businessId)
        {
            try
            {
                if (!BizList.ContainsKey(businessId)) return;

                Business biz = BizList[businessId];
                if (biz == null) return;

                // Скрытие блипа
                Main.SetBlipTransparency(biz.EnterPoint, 0);
            }
            catch (Exception e)
            {
                Log.Write($"HideBusinessBlip Exception: {e.ToString()}");
            }
        }
	}
