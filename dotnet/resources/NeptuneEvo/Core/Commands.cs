...
using System.IO;
using System.Text;
using static NeptuneEvo.Fractions.Manager;
...
[Command(AdminCommands.Giveammo)]
        ...
        [Command(AdminCommands.TestBeard)]
        public static void CMD_TestBeard(ExtPlayer player, int variation, int texture = 0)
        {
            try
            {
                // Проверяем доступ администратора к команде
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.TestBeard)) return;

                // Отправляем данные клиенту
                player.TriggerEvent("client:testBeard", variation, texture);

                // Уведомление администратора
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                    $"Тестируется борода: ID {variation}, Текстура {texture}", 5000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_TestBeard Exception: {e.ToString()}");
            }
        }
        ...
        */
        ...
        [Command(AdminCommands.NAgl)]
        public static void CMD_NAgl(ExtPlayer player, int id)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.NAgl)) return;
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                ExtPlayer target = Main.GetPlayerByID(id);
                var targetCharacterData = target.GetCharacterData();
                ...
                targetCharacterData.Licenses = new List<bool>() { false, false, false, false, false, false, false, false, false };
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) лишил комплекта лицензий {target.Name} ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы лишили комплекта лицензий {target.Name} ({target.Value})", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_Clear Exception: {e.ToString()}");
        ...
        public static void CMD_setProdAll(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "~r~Вы полностью снесли склад бизнесов!");
            }
            catch (Exception e)
            {
        ...
        */
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        bool canmats = (vehname.Equals("barracks") || vehname.Equals("cargobob") || vehname.Equals("mi8cargo") || vehname.Equals("brickade") || vehname.Equals("youga") || vehname.Equals("gburrito2") || vehname.Equals("burrito3") || vehname.Equals("gburrito") || vehname.Equals("terbyte"));
        ...
        [Command(AdminCommands.Createfracveh)]
        public static void CMD_createFractionVehicle(ExtPlayer player, string vehname, int rank, int c1, int c2)
        {
            // Получение позиции игрока для спавна транспорта
            Vector3 position = player.Position;
            Vector3 rotation = player.Rotation;
            float heading = player.Heading;

            // Создание транспортного средства
            ExtVehicle vehicle = (ExtVehicle)NAPI.Vehicle.CreateVehicle(NAPI.Util.GetHashKey(vehname), position, heading, c1, c2);
            var vehicleLocalData = vehicle.GetVehicleLocalData();
            if (vehicleLocalData != null)
            {
                if (vehicleLocalData.Access == VehicleAccess.Fraction)
                {
                    ...
                    // Установка фракционного транспорта
                    SetFracVeh(player, vehicle, vehname, rank, c1, c2);
                    var data = fractionData.Vehicles[number].customization;
                    if (!vehname.Equals(model))
                    {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        ...
                        try
                        {
                            ...
                            .Where(v => v.Number == number)
                                .Set(v => v.Model, vehname)
                                .Set(v => v.Position, JsonConvert.SerializeObject(position))
                                .Set(v => v.Rotation, JsonConvert.SerializeObject(rotation))
                                .Set(v => v.Rank, rank)
                                .Set(v => v.Colorprim, c1)
                                .Set(v => v.Colorsec, c2)
                                .Set(v => v.Components, JsonConvert.SerializeObject(data))
                        ...
                        {
                            Log.Write($"Error saving fraction vehicle to database: {e.Message}", nLog.Type.Error);
                        }
                    });

                    player.SendChatMessage($"Фракционный транспорт {vehname} успешно создан.");
                }
            }
        }
        public static void SetFracVeh1(ExtPlayer player, ExtVehicle vehicle, string vehname, int rank, int c1, int c2, bool isMessage = true)
        {


            vehicle.SetVehicleLocalData(new VehicleLocalData
            {
                Access = VehicleAccess.Fraction,
                Fraction = player.GetFractionId(),
                MinRank = rank
            });
            ...
            if (isMessage)
            {
                player.SendChatMessage($"Транспорт {vehname} успешно настроен для фракции.");
            }
        }
        ...
        public static void CMD_adminClothes(ExtPlayer player, int targetId, int clothingId, int drawable, int texture)
        {
            ExtPlayer target = (ExtPlayer)NAPI.Player.GetPlayerFromHandle(NAPI.Pools.GetAllPlayers().FirstOrDefault(p => p.Value == targetId));
            if (target == null)
            {
            ...
            InventoryItemData clothingItem = new InventoryItemData
            {
                ItemId = (ItemId)clothingId,
                Count = 1,
                Data = JsonConvert.SerializeObject(new { Drawable = drawable, Texture = texture })
            };

            // Добавление одежды в инвентарь
            target.Accessories.TryAdd(clothingId, clothingItem);
            player.SendChatMessage("Одежда успешно добавлена в инвентарь.");
        }
        ...
        public static void CMD_createVehicle(ExtPlayer player, string name, int a, int b)
        {
            try
            {
                ...
                //};
                //target.SetIntoVehicle(veh, emptySlots[0]);
        ...
        //         [Command("vehiclelist")]
        //public void GetVehiclesList(ExtPlayer player)
        //{
        //    var vehicleList = Enum.GetNames(typeof(VehicleHash));
        //    NAPI.Util.ConsoleOutput($"Всего транспортных средств: {vehicleList.Length}");

        //    foreach (var vehicle in vehicleList)
        //    {
        //        NAPI.Util.ConsoleOutput(vehicle);
        //    }

        //    player.SendChatMessage($"Список машин выведен в консоль. Всего: {vehicleList.Length}");
        //}
        //[Command("savevehiclelist")]
        //public void SaveVehiclesList(ExtPlayer player)
        //{
        //    var vehicleList = Enum.GetNames(typeof(VehicleHash));
        //    File.WriteAllLines("vehicles.txt", vehicleList);
        //    //using StreamWriter vehicleLis1t = new StreamWriter("vehicles.txt", true, Encoding.UTF8);
        //    //vehicleLis1t.WriteLine();
        //    //vehicleLis1t.Close();

        //    NAPI.Util.ConsoleOutput("✅ Список транспортных средств сохранен в vehicles.txt");
        //    player.SendChatMessage("Список машин сохранен в файле vehicles.txt.");
        //}
        ...
        //        var messages = msg.Split(new[] { "\\n" }, StringSplitOptions.None);
        //        foreach (var message in messages)
        //        {
        //            NAPI.Chat.SendChatMessageToAll($"~y~[Гос. новости]\\n {player.Name}: {message}");
        //        }
        //        GameLog.FracLog(memberFractionData.Id, characterData.UUID, -1, player.Name, "-1", $"gov({msg})");
        //        player.TriggerEvent("showGovMenu");
        //    }
        //    catch (Exception e)
        //    {
        ...
        //}
        [Command("govmenu")]
        public void CMD_OpenGovMenu(ExtPlayer player)
        {
            try
            {
                player.TriggerEvent("showGovMenu");
            ...
            {
                Log.Write($"CMD_OpenGovMenu Exception: {e.ToString()}");
            }
        }

        [RemoteEvent("sendGovMessage")]
        public void SendGovMessage(ExtPlayer player, string msg1, string msg2)
        {
            try
            {
                var sessionData = player.SessionData;
                if (sessionData == null) return;

                var characterData = player.CharacterData;
                if (characterData == null) return;

                var fractionData = player.FractionData;
                if (fractionData == null) return;
                var memberFractionData = player.GetFractionMemberData();
                if (memberFractionData == null)
                    return;
                var fracId = memberFractionData.Id;
                //string msgSender = "!{#B8962E}" + $"[{Manager.GovTags[fracId]}] {GetFractionRankName(fracId, memberFractionData.Rank)} " + player.Name.ToString().Replace('_', ' ');
                string playerName = player.GetName();
                string fractionName = fractionData.Name; // Предполагается, что у вас есть свойство Name в FractionData
                if (!Manager.GovIds.Contains(fracId)) return;
                string DepOpen = "!{#B8962E}" + $"[{GovTags[fracId]}] {GetDepartmentTag(fracId, memberFractionData.DepartmentId)} " +  player.Name.ToString().Replace('_', ' ') + " " + "Занял(а) государственную волну";
                string DepClose = "!{#B8962E}" + $"[{GovTags[fracId]}] {GetDepartmentTag(fracId, memberFractionData.DepartmentId)} " + player.Name.ToString().Replace('_', ' ') + " " + "Освободил(а) государственную волну";
                string header = $"======= [{Manager.GovTags[fracId]} {player.Name.ToString().Replace('_', ' ')}] =======";
                string footer = $"======= [С уважением, Отдел {GetDepartmentRankName(fracId, memberFractionData.DepartmentId)}] =======";

                var messages = new[] { msg1, msg2};

                for (int i = 0; i < messages.Length; i++)
                {
                    var messageLines = messages[i].Split(new[] { "\\n" }, StringSplitOptions.None);
                    foreach (var line in messageLines)
                    {
                        if (i == 0 && line == messageLines[0])
                        {
                            NAPI.Chat.SendChatMessageToAll($"{DepOpen}");
                            NAPI.Chat.SendChatMessageToAll($"~y~{header}");
                            NAPI.Chat.SendChatMessageToAll($"~y~{line}");
                            
                        }
                        else
                        {
                            NAPI.Chat.SendChatMessageToAll($"~y~{line}");
                            NAPI.Chat.SendChatMessageToAll($"~y~{footer}");
                            NAPI.Chat.SendChatMessageToAll($"{DepClose}");
                        }
                    }
                }
            }
            ...
            {
                Log.Write($"SendGovMessage Exception: {e.ToString()}", nLog.Type.Error);
                player.SendChatMessage("Произошла ошибка при отправке сообщения. Пожалуйста, попробуйте снова.");
            }
        }
        ...
        public static void CMD_fracChat1(ExtPlayer player, string msg)
        {
            try
            {
                Manager.fractionChat1(player, msg);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_fracChat1 Exception: {e.ToString()}");
            }
        }
        ...
        [Command("rb", GreedyArg = true)]
        public static void CMD_fracOOCChat1(ExtPlayer player, string msg)
        {
            try
            {
                Manager.fractionChatOOC(player, msg);
            ...
            {
                Log.Write($"CMD_fracOOCChat1 Exception: {e.ToString()}");
            }
        }
