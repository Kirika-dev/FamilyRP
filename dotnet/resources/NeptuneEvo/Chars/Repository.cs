 ...
 */
        ...
        public static IReadOnlyDictionary<ItemId, ItemsInfo> ItemsInfo = new Dictionary<ItemId, ItemsInfo>()
        {
            ...
            { ItemId.Material, new ItemsInfo("Материалы", "Нужны для создания оружия и патронов.","inv-item-fraction", "Остальное", 3045218749, 10000, new Vector3(0.0,0.0,-0.6), new Vector3(), newItemType.None) },
        ...
        public static string GetVehicleInfo(string data)
        {
            try
            {
            ...
            {
                Log.Write($"GetVehicleInfo Exception: {e.ToString()}");
            }
            return "-_-";
        }
                ...
                if (ItemsData.ContainsKey(locationName))
                {
                    foreach (string Location in ItemsData[locationName].Keys)
                    {
                        ...
                        foreach (InventoryItemData item in sortedItemData)
                        {
                            ...
                            if (newItem.ItemId == ItemId.CarKey) newItem.Data = GetVehicleInfo(newItem.Data);
        ...
        public static void PlayerStats(ExtPlayer player, ExtPlayer target = null)
        {
            try
            {
                ...
                int[] currentLevelsInfo = { 1, 1, 1, 1, 1, 1, 1, 1 };
                for (int i = 0; i <= 7; i++)
                {
