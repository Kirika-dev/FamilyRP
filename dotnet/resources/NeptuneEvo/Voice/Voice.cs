...
namespace NeptuneEvo.Voice
{
    public class Voice : Script
    {
        ...
        [Command("megaphone")]
        public static void CMD_megaphone(ExtPlayer player)
        {
            try
            {
                if (!player.IsCharacterData()) return;
                ...
                var memberFractionData = player.GetFractionMemberData();
                if (memberFractionData == null || !Voice.isGov(memberFractionData.Id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете использовать мегафон!", 3000);
                    return;
                }

                int megaphoneRange = 50; // Радиус мегафона

                if (player.HasSharedData("VoiceDist") && player.GetSharedData<int?>("VoiceDist") != null && player.GetSharedData<int>("VoiceDist") == megaphoneRange)
                {
                    player.ResetSharedData("VoiceDist");
                    Trigger.ClientEvent(player, "StopMegaphoneNoise"); // Остановить шум
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Вы отключили мегафон.", 3000);
                }
                else
                {
                    player.SetSharedData("VoiceDist", megaphoneRange);
                    Trigger.ClientEvent(player, "PlayMegaphoneNoise"); // Включить шум
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Вы включили мегафон.", 3000);
                }

            }
            catch (Exception e)
            {
                Log.Write($"CMD_megaphone Exception: {e.ToString()}");
            }
        }
