    ...
    class WorkManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(354, LangFunc.GetText(LangType.Ru, DataName.Taximany), new Vector3(899.3858, -172.74521, 74.01804), 70, true, 1.2f));
        ...
        public static List<string> JobStats = new List<string>
{
    ...
    LangFunc.GetText(LangType.Ru, DataName.CargoDelivery), // Добавляем новую работу
};
        ...
        public static SortedList<int, Vector3> Points = new SortedList<int, Vector3>
        {
            ...
            {3, new Vector3(899.3858, -172.74521, 74.01804) },      // Taxi job
            {4, new Vector3(435.23758, -653.01117, 28.730534) }, // Bus driver job
            {5, new Vector3(-1330.482, 42.12986, 53.48915) },  // Lawnmower job
            {6, new Vector3(1018.9361, -2511.5688, 28.480877) },  // Trucker job
            {7, new Vector3(-196.52617, -830.6757, 30.771637) },  // Collector job
            {8, new Vector3(-229.61082, -1378.2086, 31.258224) },  // AutoMechanic job
            {9, new Vector3(436.5074, -627.4617, 28.707539) },
        };
        private static readonly SortedList<int, string> JobList = new SortedList<int, string>
{
    ...
    {9, LangFunc.GetText(LangType.Ru, DataName.CargoDelivery) } // Добавляем новую работу
};
        ...
        public static SortedList<int, int> JobsMinLvl = new SortedList<int, int>()
        {
            ...
            { (int) JobsId.CargoDelivery, 1 },
        };// Минимальный уровень для работы Cargo Delivery
        public static void JobJoin(ExtPlayer player, int job)
        {
            try
            {
                ...
                else
                {
                    ...
                    if ((job == (int)JobsId.CargoDelivery) && !characterData.Licenses[2])
                    {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.JobLicC), 10000);
                    return;
                    }
