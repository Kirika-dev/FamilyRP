...
namespace NeptuneEvo.Fractions
{
    class Manager : Script
    {
        ...
        public static void onResourceStart()
        {
            try
            {
                Main.CreateBlip(new Main.BlipData(487, "Merryweather Security", new Vector3(-144.3048, -593.54, 211.76), 4, true, 1.5f));
        ...
        public static Vector3[] ArmySpawn = new Vector3[3]
        {
            new Vector3(-2361.488, 3208.3765, 29.224823),
            new Vector3(-2466.209, 3315.813, 30.977722),
            new Vector3(-2358.195, 3252.433, 30.80552)
        };
        ...
        public static void fractionChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                string msgSender = $"~b~[F] {GetDepartmentTag(fracId, memberFractionData.DepartmentId)}{GetDepartmentRankName(fracId, memberFractionData.DepartmentRank)} {GetFractionRankName(fracId, memberFractionData.Rank)} " + player.Name.ToString().Replace('_', ' ') + " (" + player.Value + "): " + Commands.RainbowExploit(message);
        ...
        public static void fractionChat1(ExtPlayer player, string message)
        {
            try
            {
                var characterData = player.GetCharacterData();
                ...
                string msgSender = $"~r~[R] {GetDepartmentTag(fracId, memberFractionData.DepartmentId)} {GetFractionRankName(fracId, memberFractionData.Rank)} " +
                                   $"~c~{player.Name.ToString().Replace('_', ' ')} (~y~{player.Value}~w~): " +
                                   $"~b~{Commands.RainbowExploit(message)}";
                ...
                foreach (ExtPlayer foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    if (foreachMemberFractionData.Id == fracId)
                    {
                        ...
                        if (Configs.IsFractionPolic(fracId) || fracId == (int)Models.Fractions.ARMY)
                            foreachPlayer.Eval("mp.game.audio.playPoliceReport(\"SCRIPTED_SCANNER_REPORT_MIC_AMANDA_01\", 0.0);"); // Police Report Sound for LSPD & FBI
                    }
                }
                GameLog.AddInfo($"(FChat({fracId})) player({characterData.UUID}) {message}");
            }
        ...
        public static void govFractionChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                string msgSender = "!{#B8962E}" + $"[{GovTags[fracId]}] {GetDepartmentTag(fracId, memberFractionData.DepartmentId)} " + player.Name.ToString().Replace('_', ' ') +" " + Commands.RainbowExploit(message);
                foreach (ExtPlayer foreachPlayer in Character.Repository.GetPlayers())
                {
                    var foreachMemberFractionData = foreachPlayer.GetFractionMemberData();
                    if (foreachMemberFractionData == null)
        ...
        public static string GetDepartmentTag(int fractionId, int departmentId)
        {
            var fractionData = Manager.GetFractionData(fractionId);
        ...
        public static string GetDepartmentRankName(int fractionId, int departmentRank)
        {
            var fractionData = Manager.GetFractionData(fractionId);
            if (fractionData != null && departmentRank > 0 && fractionData.Departments.ContainsKey(departmentRank))
                return fractionData.Departments[departmentRank].Name;
            ...
            try
            {
                ...
                if (player.Position.DistanceTo(Police.GunsPosition) >= 5 &&
                    ...
                    player.Position.DistanceTo(Army.ArmyCheckpoints[0]) >= 5 &&
                     player.Position.DistanceTo(Army.ArmyCheckpoints[9]) >= 5 &&
                      player.Position.DistanceTo(Army.ArmyCheckpoints[10]) >= 5 &&
    ...
    class Stocks : Script
    {
        public static int MaxCargobobMats = 1300000;
        ...
        public static Dictionary<int, Vector3> matsCoords = new Dictionary<int, Vector3>()
        {
            ...
            { (int) Models.Fractions.ARMY, new Vector3(-2053.025, 3094.7625, 31.810448)},
            { (int) Models.Fractions.LSNEWS, new Vector3()},
        ...
        {
            { (uint)VehicleHash.Barracks, 130000 },
            { (uint)VehicleHash.Brickade, 130000 },
            { (uint)VehicleHash.Cargobob, 130000 },
            { (uint)VehicleHash1.mi8cargo, 250000 },
            { (uint)VehicleHash.Burrito3, 50000 },
            { (uint)VehicleHash.Youga, 50000 },
            { (uint)VehicleHash.Gburrito, 50000 },
            { (uint)VehicleHash.Terbyte, 100000 },
        };
        ...
        public static void OnWar(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    sessionData.TimersData.LoadMatsTimer = Timers.StartOnce(10000, () => Army.loadMaterialsTimer(player), true);
