...
namespace NeptuneEvo.Fractions
{
    class Police : Script
    {
        ...
        private static Vector3 policeZoneCenter = new Vector3(460, -990, 30);
        float policeZoneRadius = 100f; // Радиус 100 метров
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                CustomColShape.CreateCylinderColShape(VehicleRepairPosition, 3, 5, 0, ColShapeEnums.FractionPolic, 9);
                ...
                NAPI.Util.ConsoleOutput("[Police] Колшэйп территории ПД загружен!");
            }
            catch (Exception e)
            {
        ...
        private void OnEntityEnterPoliceZone(ColShape shape, ExtPlayer player)
        {
            try
            {
                if (player.Dimension == 0)
                {
                    player.Dimension = 1; // Переносим в Dimension 1
                    player.SendChatMessage("Вы вошли на территорию полицейского участка!");
                    player.SetData("PoliceZoneMarker", NAPI.Marker.CreateMarker(1, player.Position, new Vector3(), new Vector3(), 1, new Color(0, 255, 0, 220)));
                    player.SetData("PoliceZoneTextLabel", NAPI.TextLabel.CreateTextLabel("Вы находитесь на территории полицейского участка", player.Position, 5F, 0.3F, 0, new Color(255, 255, 255)));

                    // Вызов клиентского события для отображения эффекта при входе
                    NAPI.ClientEvent.TriggerClientEvent(player, "showEffect", "MP_Celeb_Win", 0, 0, 0, 0, 0, 0, 0);
                    NAPI.ClientEvent.TriggerClientEvent(player, "playSound", "Mission_Pass_Notify");

                    Log.Write($"Player {player.Name} entered the police zone.");
                }
            }
            catch (Exception e)
            {
                Log.Write($"OnEntityEnterPoliceZone Exception: {e}");
            }
        }

        private void OnEntityExitPoliceZone(ColShape shape, ExtPlayer player)
        {
            try
            {
                if (player.Dimension == 1)
                {
                    player.Dimension = 0; // Возвращаем в обычный мир
                    player.SendChatMessage("Вы покинули территорию полицейского участка!");
                    if (player.HasData("PoliceZoneMarker"))
                    {
                        player.GetData<Marker>("PoliceZoneMarker").Delete();
                        player.ResetData("PoliceZoneMarker");
                    }
                    if (player.HasData("PoliceZoneTextLabel"))
                    {
                        player.GetData<TextLabel>("PoliceZoneTextLabel").Delete();
                        player.ResetData("PoliceZoneTextLabel");
                    }

                    // Вызов клиентского события для отображения эффекта при выходе
                    NAPI.ClientEvent.TriggerClientEvent(player, "showEffect", "MP_Celeb_Lose", 0, 0, 0, 0, 0, 0, 0);
                    NAPI.ClientEvent.TriggerClientEvent(player, "playSound", "Mission_Fail_Notify");

                    Log.Write($"Player {player.Name} exited the police zone.");
                }
            }
            catch (Exception e)
            {
                Log.Write($"OnEntityExitPoliceZone Exception: {e}");
            }
        }
                ...
                catch
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            var wanted = JsonConvert.DeserializeObject<Character.Models.WantedLevel>(character.Wanted);
        ...
        public static void setPlayerWantedLevel(ExtPlayer player, Character.Models.WantedLevel wantedlevel)
        {
            try
            {
                var characterData = player.GetCharacterData();
