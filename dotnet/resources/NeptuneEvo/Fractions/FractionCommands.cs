    ...
    class FractionCommands : Script
    {
        ...
        public static void ticketToTarget(ExtPlayer player, ExtPlayer target, int sum, string reason)
        {
            try
            {
                ...
                Table.Logs.Repository.AddLogs(player, FractionLogsType.Ticket, $"Выписал штраф {target.Name} ({targetCharacterData.UUID})");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (player != target)
                {
                    ...
                    if (targetCharacterData.WantedLVL == null || targetCharacterData.WantedLVL.Level + stars <= 6)
                    {
                        ...
                        Character.Models.WantedLevel wantedLevel = new Character.Models.WantedLevel(oldStars + stars, player.Name, DateTime.Now, reason);
