...
namespace NeptuneEvo.Fractions
{
    class Configs : Script
    {
        ...
        public static void InitFractionDefaultAccessRanks()
        {
            ...
            fractionData.RanksDefaultAccess.Add(20, new List<RankToAccess>() { RankToAccess.OpenWeaponStock, RankToAccess.Su, RankToAccess.TakeGunLic, RankToAccess.Ticket, RankToAccess.InCar, RankToAccess.Pull, RankToAccess.Cuff, RankToAccess.Follow, RankToAccess.Dep, RankToAccess.Invite, RankToAccess.UnInvite, RankToAccess.Gov, RankToAccess.SetRank, RankToAccess.OpenStock, RankToAccess.Dron });
        ...
        public static void InitFractionDefaultRanksName()
        {
            ...
            AddFractionRanksData(fractionId, 16, "Генерал Майор", 1000, 16, fractionData.RanksDefaultAccess[16]);
            AddFractionRanksData(fractionId, 17, "Генерал Лейтенант", 1000, 17, fractionData.RanksDefaultAccess[17]);
            AddFractionRanksData(fractionId, 18, "Генерал Полковник", 1000, 18, fractionData.RanksDefaultAccess[18]);
            AddFractionRanksData(fractionId, 19, "Генерал Армии", 1000, 19, fractionData.RanksDefaultAccess[19]);
            AddFractionRanksData(fractionId, 20, "Маршал", 1000, 20, fractionData.RanksDefaultAccess[20]);
            AddFractionRanksData(fractionId, 21, "СуперМаршал", 1000, 21, fractionData.RanksDefaultAccess[21]);
            ...
            try
            {
                ...
                if (resultFraction != null && resultFraction.Rows.Count > 0)
                {
                    foreach (DataRow dataRow in resultFraction.Rows)
                    {
                        ...
                        CustomColShape.CreateCylinderColShape(Stocks.matsCoords[fractionId], 30, 50, 0, ColShapeEnums.FractionStock, fractionId, 1); // garage colshape
                        NAPI.Marker.CreateMarker(1, Stocks.matsCoords[fractionId], new Vector3(), new Vector3(), 5f, new Color(155, 0, 0, 128));
            ...
            try
            {
                foreach (var fractionData in Manager.Fractions)
                {
                    foreach (var vehicle in fractionData.Value.Vehicles)
                    {
                        ...
                        bool canmats = (vehname.Equals("barracks") || vehname.Equals("cargobob") || vehname.Equals("mi8cargo") ||
