...
namespace NeptuneEvo.Fractions
{
    class Ems : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                //NAPI.TextLabel.CreateTextLabel("~w~Нина Сердешная", new Vector3(349.9646, -1429.8965, 32.50935), 5f, 0.3f, 4, new Color(255, 255, 255), true, NAPI.GlobalDimension);                
                NAPI.TextLabel.CreateTextLabel("~w~Жорик Парковый", new Vector3(333.86792, -1428.573, 32.50929), 0f, 0.3f, 4, new Color(255, 255, 255), true, NAPI.GlobalDimension);

                #region cols
                //CustomColShape.CreateCylinderColShape(emsCheckpoints[3], 1, 1f, 0, ColShapeEnums.FractionEms, 1); // open ho spital stock
                //NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Открыть склад"), new Vector3(emsCheckpoints[3].X, emsCheckpoints[3].Y, emsCheckpoints[3].Z), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                PedSystem.Repository.CreateQuest("s_m_m_scientist_01", new Vector3(359.27008, -1414.6313, 32.509293), 47.939796f, title: "~y~NPC~w~ Эммануэль\nДежурный врач", colShapeEnums: ColShapeEnums.FracEms);
                PedSystem.Repository.CreateQuest("s_m_y_autopsy_01", new Vector3(364.43396, -1413.6874, 32.509293), -46.78159f, title: "~y~NPC~w~ Михаил\nВетеринар", colShapeEnums: ColShapeEnums.FracEmsVeterinarian);
                PedSystem.Repository.CreateQuest("s_m_m_doctor_01", new Vector3(357.38092, -1416.8903, 32.509293), 47.802277f, title: "~y~NPC~w~ Мед.брат Станислав\nВызвать сотрудника", colShapeEnums: ColShapeEnums.CallEmsMember);
               
                PedSystem.Repository.CreateQuest("kost9", new Vector3(349.9646, -1429.8965, 32.50935), -45.4007f, title: "~y~NPC~w~ Нина Сердешная\nОткрыть склад", colShapeEnums: ColShapeEnums.Sklad);
            }
            catch (Exception e)
            {
        ...
        public static Vector3[] emsCheckpoints = new Vector3[10]
        {
            new Vector3(297.672, -583.996, 43.26), // ems blip       0
            new Vector3(328.5222, -1409.3462, 37.91115), // spawn after death      1
            new Vector3(320.97617, -1404.1388, 37.91112), // spawn after death      2
            //new Vector3(350.6195, -1428.9807, 32.509354), // open hospital stock   3
            new Vector3(311.47562, -1409.052, 32.509308), // duty change            4
        ...
        public static string OnCallEms(ExtPlayer player, bool death = false)
        {
            try
            {
                ...
                if (!death)
                {
                    ...
                    sessionData.TimingsData.NextCallEMS = DateTime.Now.AddMinutes(3);
                }
                else
                {
                ...
                try
                {
                    if (killerCharacterData != null && killerSessionData != null && !Voice.Voice.isGov(entityKiller.GetFractionId()) && player != entityKiller)
                    {
                        if ((!entityKiller.HasSharedData("IS_MASK") || !entityKiller.GetSharedData<bool>("IS_MASK")) && (killerSessionData.InAirsoftLobby == -1) && killerSessionData.InTanksLobby == -1 && !killerSessionData.WarData.IsWarZone)
                        {
                            ...
                            Character.Models.WantedLevel wantedLevel = new Character.Models.WantedLevel(correctLevel, LangFunc.GetText(LangType.Ru, DataName.Police), DateTime.Now, LangFunc.GetText(LangType.Ru, DataName.Murder));
        ...
        public static void ReviveFunc(ExtPlayer player, bool reviveonplace = false)
        {
            try
            {
                ...
                if (!reviveonplace)
                {
                    ...
                    //    spawnDimension = 0;//3244522;
                    //}
                    else if (fracId == (int)Models.Fractions.ARMY)
                    {
                        spawnPos = Manager.ArmySpawn[Main.rnd.Next(3)] + new Vector3(0, 0, 1.12);
                        spawnDimension = 0;//3244522;
                    }
        ...
        [Interaction(ColShapeEnums.Sklad)]
        public static void OnSklad(ExtPlayer player)
        {
            try
            {
                var sessionData = player.GetSessionData();
            ...
            {
                Log.Write($"OnSkad Exception: {e.ToString()}");
            }
        }
